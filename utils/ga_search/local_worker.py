...
def main():
    ...
    try:
        ...
        if local_device != "cpu" and not torch.cuda.is_available():
            raise RuntimeError(
                f"CUDA was requested with local_device={local_device}, "
                "but torch.cuda.is_available() is False. "
                "Check NVIDIA driver, CUDA_VISIBLE_DEVICES, LD_LIBRARY_PATH, "
                "and the conda PyTorch CUDA build."
            )
        ...
        # Normalize tokenizer output across transformers versions.
        # Qwen chat templates may return Tensor, dict, or BatchEncoding/UserDict.
        if hasattr(encoded, "keys") and not torch.is_tensor(encoded):
            generate_inputs = {}
            for key in encoded.keys():
                value = encoded[key]
                generate_inputs[key] = value.to(model_device) if hasattr(value, "to") else value
            input_ids = generate_inputs.get("input_ids")
            if input_ids is None:
                raise RuntimeError(
                    f"tokenizer output does not contain input_ids: {list(generate_inputs.keys())}"
                )
        ...
        completion_tokens = max(0, generated.shape[-1] - input_ids.shape[-1])
